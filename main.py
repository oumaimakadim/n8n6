...
def extract_val_info(pattern, text):
    ...
    if match:
        ...
        if raw_str in ["-", "–", "—"]: return 0.0, False, "0"
...
async def parse_pdf(file: UploadFile = File(...)):
    ...
    # محاولة 1: استهداف الجملة اللي فالتصويرة بالحرف
    # كنقلبو على "RegCom number of voting sheet" ونقبلو أي مسافات
    specific_match = re.search(r"RegCom\s+number\s+of\s+voting\s+sheet\s*[:\.]?\s*([A-Za-z0-9\-/]+)", text, re.IGNORECASE)
    
    if specific_match:
        voting_sheet = specific_match.group(1).strip()
    else:
        # محاولة 2: البحث عن الكود Vxxxx/xx فين ما كان
        v_match = re.search(r"\b([V]\s*\d{3,8}\s*/\s*\d{2})\b", text, re.IGNORECASE)
        voting_sheet = v_match.group(1).replace(" ", "").upper() if v_match else ""

    # Draft Number
    # نفس الشيء، كنستهدفو الجملة كاملة
    draft_match_text = re.search(r"RegCom\s+number\s+of\s+draft\s+implementing\s+act/measure\s*[:\.]?\s*(.*)", text, re.IGNORECASE)
    if draft_match_text:
        draft_number = draft_match_text.group(1).strip()
    else:
        # محاولة 2
        d_match = re.search(r"\b([D]\s*\d{3,8}\s*/\s*\d{2})\b", text, re.IGNORECASE)
        draft_number = d_match.group(1).replace(" ", "").upper() if d_match else ""
