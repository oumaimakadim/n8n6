...
def fmt(val, is_pct):
    ...
    # تنسيق الأرقام الكبيرة (فواصل الآلاف)
...
async def parse_pdf(file: UploadFile = File(...)):
    ...
    # --- 3. Logic Fixes (V8 - The Robust Logic) ---
    ...
    # القيم النهائية للعرض
    pop_for_out, pop_ag_out, pop_abs_out = "", "", ""

    has_any_data = (pop_for is not None or pop_ag is not None or pop_abs is not None)
    
    if has_any_data:
        # القاعدة الجديدة: هل هناك أي رقم أكبر من 100؟
        # نستخدم القيم المعوضة بـ 0.0 لتفادي الأخطاء
        v_for = pop_for if pop_for is not None else 0.0
        v_ag = pop_ag if pop_ag is not None else 0.0
        v_abs = pop_abs if pop_abs is not None else 0.0

        is_absolute_mode = (v_for > 100 or v_ag > 100 or v_abs > 100)
        
        if is_absolute_mode:
            # === حالة الأرقام الكبيرة (Absolute) ===
            final_absent_pop = ""  # العميل قال خليها خاوية
            
            # نحسب المجموع (Sum) كما طلب العميل
            ...
            # === حالة النسب المئوية (Percentages) ===
            # هنا كنحسبو Absent ديما، حيت المجموع خاصو يكون 100
            calc_absent = 100.0 - (v_for + v_ag + v_abs)
            
            # تصحيح أخطاء الفواصل الصغيرة
            if calc_absent < 0.01: calc_absent = 0.0
            
            final_absent_pop = f"{calc_absent:.2f}%"
            final_sum_pop = "100.00%" # العميل طلب المجموع يكون عامر ديما
