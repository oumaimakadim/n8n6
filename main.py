...
def extract_val_info(pattern, text):
    """
    تستخرج القيمة وتعيد: (القيمة_كرقم, هل_هي_نسبة, النص_الأصلي)
    """
    match = re.search(pattern, text, re.IGNORECASE | re.DOTALL)
    if match:
        raw_str = match.group(1).strip()
        
        # 1. حالة الشرطة "-" تعني 0
        if raw_str in ["-", "–", "—"]:
            return 0.0, False, "0"
...
async def parse_pdf(file: UploadFile = File(...)):
    ...
    # --- 1. Metadata (UPDATED REGEX) ---
    
    # Voting Sheet: البحث عن الكود مباشرة (Vxxxx/xx) أو عن طريق السياق
    # هذا الـ Regex يصيد V112033/01 بدقة
    vs_match = re.search(r"\b([V]\s*\d{3,8}\s*/\s*\d{2})\b", text, re.IGNORECASE)
    if vs_match:
        voting_sheet = vs_match.group(1).replace(" ", "").upper()
    else:
        # محاولة احتياطية: البحث وراء كلمة voting sheet (مع تجاهل RegCom)
    ...
    # Draft Number: البحث عن الكود (Dxxxx/xx)
    ...
    if not date_opinion:
        # بحث عام عن التاريخ في الملف إذا لم نجده في السياق
    ...
    # تعويض القيم الناقصة بـ 0 فقط إذا كان عدد الدول المقابل لها هو 0
    ...
    if has_any_pop_data:
        ...
        if is_pct_mode:
            # حالة النسب المئوية: نحسب Absent
            ...
            # حالة الأرقام المطلقة: لا نحسب Absent
            final_absent_pop = "" 
    ...
    # Sum Numbers (States) - يحسب دائماً إذا توفرت الأرقام
